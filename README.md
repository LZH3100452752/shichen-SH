...
| L3 | 常规清场 | 禁用全部非白名单模块 |
| L4 | 深度清场 | 禁用全部模块（含白名单），只保留守护自身 |
| L5 | 终极隔离 | 重命名模块目录，彻底移除挂载 |
...
## 十、v1.3 交互反馈（加载 / 过渡 / 进度）

- **分步加载遮罩**：执行耗时操作时全屏展示进度环 + 百分比 + 进度条 + 步骤清单，步骤逐条点亮并打勾
- **顶部不确定进度条**：任何后台请求都会在顶部走一道流光，不用盯着空白等
- **骨架屏**：首屏、防护页、冲突页、日志、终端在加载中显示占位块，而不是一片空白
- **扫描线动效**：脚本检测过程中显示来回扫描的光带
- **按钮内联加载态**：命令执行时按钮自带转圈
- **结果渐入 / 页面切换过渡**：检测结果卡片依次淡入，切页有位移过渡
- **无障碍**：开启系统「减少动画」时所有动效自动关闭

同时修复了两个真实缺陷：
1. 切到「冲突 / 防护 / 日志」页时自动加载的回调引用错误，导致数据一直停在"加载中"
2. `guard()` 未返回 Promise，使首屏 `load().catch()` 抛错、失败时不会显示错误卡片
...
- **进度显示修复**：百分比改用 `translateY` 数字滚轮（合成线程），此前用 `@pr